  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -s -L https://brave-browser-apt-release.s3.brave.com/dists/stable/main/binary-amd64/Packages.gz | gunzip |grep -A 7 -m 1 'Package: brave' | awk -F ': ' '/Version/{print $2;exit}' ''',
            returnStdout: true).trim()
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.description=[The Brave browser](https://brave.com/) is a fast, private and secure web browser for PC, Mac and mobile.\" \
